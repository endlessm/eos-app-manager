...
#
# Parameters:
# <app_id>: ID of the application which update failed.
#
...
if [ ! -d "${APP_PREFIX}" ]; then
  debug "As the '${APP_PREFIX}' directory does not exist, there are no symbolic links to delete"
  debug "because the symbolic links are always removed before deleting the directory, "
  debug "and always created after creating the directory."
